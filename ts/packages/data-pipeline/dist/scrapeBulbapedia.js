...
export function scrapeBulbapediaCard(html, cardId) {
    ...
    if (!nameMatch)
        throw new Error("Could not parse card name");
    return {
        name: nameMatch[1],
        hp: hpMatch ? parseInt(hpMatch[1]) : undefined,
        stage: stageMatch ? parseInt(stageMatch[1]) : 0,
...
function getCacheKey(cardName) {
    return createHash("md5").update(cardName).digest("hex") + ".html";
}
function getCachedHtml(cardName) {
    try {
...
function setCachedHtml(cardName, html) {
    try {
...
export async function scrapeBulbapediaCards(cardNames) {
    const results = [];
    const failures = [];
                ...
                try {
                    ...
                    if (!response.ok) {
                        ...
                        if (retryable && attempt < RETRY_ATTEMPTS) {
                            console.warn(`  Attempt ${attempt}/${RETRY_ATTEMPTS}: ${lastError}, retrying...`);
...
//# sourceMappingURL=scrapeBulbapedia.js.map
...
import { Decision, Policy, playFrom } from "./episode.js";

/** Uniform over legal actions. The floor any learned policy has to beat. */
export const randomPolicy = (rng: () => number = Math.random): Policy => ({ space }: Decision) => {
  const legal: number[] = [];
  for (let i = 0; i < space.mask.length; i++) if (space.mask[i]) legal.push(i);
 ...
 */
export const heuristicPolicy = (rng: () => number = Math.random): Policy => ({ state, seat, space }: Decision) => {
  let bestIndex = -1;
...
function scoreAction(state: GameState, seat: Player, action: Action): number {
  ...
  switch (action.type) {
    ...
    case "attack": {
      ...
      if (!defender || !defenderDef) return 0;
    ...
    case "playTrainer": {
      ...
      if (def?.type !== "trainer") return 100;
      // Supporters are once per turn, so spend them before Items.
      if (def.subtype === "supporter") return 300;
      if (def.subtype === "tool") return 150;
      if (def.subtype === "stadium") return 120;
      return 200;
    }
...
function promoteValue(state: GameState, seat: Player, instanceId: string): number {
  const ps = state.players[seat];
  const card =
    ps.bench.find((p) => p.card.instanceId === instanceId)?.card ??
...
function chooseValue(state: GameState, seat: Player, action: Extract<Action, { type: "choose" }>): number {
  const choice = state.pendingChoice;
  if (!choice) return 0;
  // Declining is a last resort: cards are usually played for their effect.
  if (action.instanceId === undefined) return -50;
  ...
  if (/Discard/.test(choice.prompt)) {
    if (def.type === "energy") return -20;
    if (def.type === "pokemon") return -10;
    return 0;
  }
  // Otherwise prefer Pokémon that can attack, then Energy, then Trainers.
  if (def.type === "pokemon") return 20 + (def.prizeValue ?? 1) * 5;
  if (def.type === "energy") return 15;
  return 10;
}
...
export const rolloutPolicy = (
  options: { candidates?: number; rollouts?: number; rng?: () => number } = {}
): Policy => {
  ...
  return (decision: Decision): number => {
    ...
    for (const { index } of scored) {
      ...
      for (let r = 0; r < rollouts; r++) {
        ...
        else if (!result.winner) wins += 0.5; // unfinished: treat as a draw
      }

...
function playSlotOf(state: GameState, player: Player, instanceId: string): number {
  const ps = state.players[player];
  if (ps.active?.card.instanceId === instanceId) return 0;
  const bench = ps.bench.findIndex((p) => p.card.instanceId === instanceId);
  return bench < 0 ? -1 : bench + 1;
}

function handSlotOf(state: GameState, player: Player, cardId: string): number {
 ...
 */
export function encodeAction(state: GameState, action: Action): number {
  ...
  switch (action.type) {
    ...
    case "playTrainer": {
      const hand = handSlotOf(state, player, action.cardId);
      if (hand < 0 || hand >= HAND_SLOTS) return -1;
    ...
    case "useAbility": {
      ...
      const abilityIndex =
        def && def.type === "pokemon" ? def.abilities.findIndex((a) => a.name === action.abilityName) : -1;
    ...
    case "choose": {
      ...
      if (!choice) return -1;
      if (action.instanceId === undefined) return OFFSETS.choose + CHOICE_SLOTS;
...
export function actionSpace(state: GameState, seat: Player): ActionSpaceView {
  const mask = new Uint8Array(ACTION_SPACE_SIZE);
  const actions = new Array<Action | undefined>(ACTION_SPACE_SIZE);

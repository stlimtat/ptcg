 ...
 */
export const promoteHandler: ActionHandler = {
  isLegal(state: GameState, action: Action): boolean {
    if (action.type !== "promote") return false;
    if (!state.pendingPromote?.includes(action.player)) return false;
    if (state.players[action.player].active) return false;

 ...
 */
function checkup(state: GameState, player: Player): GameState {
  ...
  if (conditions.includes("Poisoned")) {
    ...
    if (!next.players[player].active) return next; // knocked out by poison
  }

  if (conditions.includes("Burned")) {
    next = applyDamage(next, player, 20, null);
    if (!next.players[player].active) return next;
    const [heads, afterFlip] = flipCoin(next);
    next = afterFlip;
    if (heads) conditions = conditions.filter((c) => c !== "Burned");
  }

  if (conditions.includes("Asleep")) {
    const [heads, afterFlip] = flipCoin(next);
    next = afterFlip;
    if (heads) conditions = conditions.filter((c) => c !== "Asleep");
  }

  conditions = conditions.filter((c) => c !== "Paralyzed");

  const current = next.players[player].active as PokemonInPlay;
  return {
    ...next,
    players: {
      ...next.players,
...
export const endTurnHandler: ActionHandler = {
  ...
  apply(state: GameState, action: Action): GameState {
    const player = action.player as Player;

import { Attack, Card, GameState, Player, PokemonInPlay, PokemonType } from "../types.js";
...
const inPlay = (ps: GameState["players"][Player]): PokemonInPlay[] =>
  [ps.active, ...ps.bench].filter((p): p is PokemonInPlay => !!p);

const stadiumName = (state: GameState): string | null =>
  state.stadium ? getCard(state, state.stadium.cardId)?.name ?? null : null;

/** Jamming Tower switches every Tool off. */
export const toolsDisabled = (state: GameState): boolean => stadiumName(state) === "Jamming Tower";

function toolNames(state: GameState, poke: PokemonInPlay): string[] {
  if (toolsDisabled(state)) return [];
  return poke.attachedTools.map((t) => getCard(state, t.cardId)?.name).filter((n): n is string => !!n);
}

const subtypesOf = (def: Card | null): string[] => ((def as any)?.subtypes ?? []) as string[];
const isTera = (def: Card | null) => subtypesOf(def).some((s) => /Tera/i.test(s));
const hasRuleBox = (def: Card | null) =>
  !!def && def.type === "pokemon" && ((def.prizeValue ?? 1) > 1 || subtypesOf(def).some((s) => /^(ex|V|VMAX|VSTAR|Radiant|ACE SPEC)$/i.test(s)));
 ...
 */
export function abilitiesLocked(state: GameState, poke: PokemonInPlay): boolean {
  const def = getPokemon(state, poke.card.cardId);
  if (!def) return false;
  if (stadiumName(state) === "Team Rocket's Watchtower" && def.types.includes("Colorless")) return true;
  return false;
}

/** An ability that is simply on while its Pokémon is in play. */
function continuousAbilityInPlay(state: GameState, owner: Player, abilityName: string): boolean {
  return inPlay(state.players[owner]).some((poke) => {
    if (abilitiesLocked(state, poke)) return false;
    const def = getPokemon(state, poke.card.cardId);
    return def?.abilities.some((a) => a.name === abilityName) ?? false;
  });
}

export function effectiveHp(state: GameState, poke: PokemonInPlay): number {
  const def = getPokemon(state, poke.card.cardId);
  let hp = def?.hp ?? 1;
  for (const tool of toolNames(state, poke)) {
    if (tool === "Hero's Cape") hp += 100;
    if (tool === "Bravery Charm") hp += 50;
  }
  return hp;
}

export function effectiveRetreatCost(state: GameState, owner: Player, poke: PokemonInPlay): number {
  const def = getPokemon(state, poke.card.cardId);
  let cost = def?.retreatCost ?? 0;

  // Latias ex — your Basic Pokémon have no Retreat Cost.
  if (def?.stage === 0 && continuousAbilityInPlay(state, owner, "Skyliner")) return 0;

  for (const tool of toolNames(state, poke)) {
    if (tool === "Air Balloon") cost -= 2;
    if (tool === "Escape Rope") cost -= 1;
  }
 ...
 */
export function effectiveWeakness(
  state: GameState,
  owner: Player,
  poke: PokemonInPlay
): { type: PokemonType; mult: 2 } | undefined {
  const def = getPokemon(state, poke.card.cardId);
...
export function attackDamageBonus(
  ...
  for (const tool of toolNames(state, source)) {
    if (tool === "Brave Bangle" && !hasRuleBox(def)) bonus += 30;
    if (tool === "Hop's Choice Band" && def?.name.startsWith("Hop's")) bonus += 30;
    if (tool === "Maximum Belt" && subtypesOf(targetDef).includes("ex")) bonus += 50;
    if (tool === "Choice Belt" && hasRuleBox(targetDef)) bonus += 30;
    if (tool === "Vitality Band") bonus += 20;
  ...
  if (stadiumName(state) === "Postwick" && def?.name.startsWith("Hop's")) bonus += 30;

  return bonus;
}

/** Damage reduction applied to the Pokémon taking the hit. */
export function damageTakenReduction(state: GameState, poke: PokemonInPlay): number {
  let reduction = 0;
  for (const tool of toolNames(state, poke)) {
    if (tool === "Rock Chestplate") reduction += 30;
    if (tool === "Bravery Charm") reduction += 0; // HP only
  }
  return reduction;
}
 ...
 * attack cheaper.
 */
export function attackCostAdjustment(state: GameState, attacker: Player, source: PokemonInPlay): number {
  const def = getPokemon(state, source.card.cardId);
  let adjustment = 0;

  for (const tool of toolNames(state, source)) {
    if (tool === "Hop's Choice Band" && def?.name.startsWith("Hop's")) adjustment -= 1;
    if (
      tool === "Counter Gain" &&
  ...
  if (stadiumName(state) === "Nighttime Mine" && isTera(def)) adjustment += 1;

  return adjustment;
}

/** Area Zero Underdepths widens the Bench for players with a Tera Pokémon. */
export function benchLimit(state: GameState, owner: Player): number {
...
export const benchCountersPrevented = (state: GameState): boolean => stadiumName(state) === "Battle Cage";

/** Festival Grounds keeps Special Conditions off anything with Energy attached. */
export function immuneToStatus(state: GameState, poke: PokemonInPlay): boolean {
...
export function benchPlacementDamage(state: GameState, cardId: string): number {
  if (stadiumName(state) !== "Risky Ruins") return 0;
  const def = getPokemon(state, cardId);
  if (!def || def.stage !== 0 || def.types.includes("Darkness")) return 0;
  return 20;
}

/** Forest of Vitality lets Grass Pokémon evolve the turn they are played. */
export function canEvolveImmediately(state: GameState, cardId: string): boolean {
  if (stadiumName(state) !== "Forest of Vitality") return false;

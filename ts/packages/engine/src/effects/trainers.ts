import { Card, CardInstance, GameState, Player, PokemonInPlay } from "../types.js";
...
function removeFromZones(state: GameState, player: Player, ids: Set<string>): GameState {
  const ps = state.players[player];
  const keep = (c: CardInstance) => !ids.has(c.instanceId);
...
const isPokemon = (state: GameState, c: CardInstance) => getCard(state, c.cardId)?.type === "pokemon";
const isBasicEnergy = (state: GameState, c: CardInstance) => {
  const def = getCard(state, c.cardId) as (Card & { basic?: boolean }) | null;
  return def?.type === "energy" && def.basic === true;
};

/** ex / V / Radiant / ACE SPEC cards all have a Rule Box. */
export function hasRuleBox(state: GameState, c: CardInstance): boolean {
  const def = getCard(state, c.cardId) as any;
  if (!def) return false;
  if ((def.prizeValue ?? 1) > 1) return true;
  return (def.subtypes ?? []).some((s: string) => /^(ex|V|VMAX|VSTAR|Radiant|ACE SPEC)$/i.test(s));
}

const pokemonInPlay = (ps: GameState["players"][Player]): PokemonInPlay[] =>
  [ps.active, ...ps.bench].filter((p): p is PokemonInPlay => !!p);

function attachEnergyTo(state: GameState, player: Player, energy: CardInstance, targetId: string): GameState {
  const ps = state.players[player];
  const add = (p: PokemonInPlay) =>
    p.card.instanceId === targetId ? { ...p, attachedEnergy: [...p.attachedEnergy, energy] } : p;
  return patch(state, player, {
...
function benchPokemon(state: GameState, player: Player, cards: CardInstance[]): GameState {
  const ps = state.players[player];
...
const searchDeckToHand = (
  effect: string,
  prompt: string,
  filter: (state: GameState, c: CardInstance) => boolean,
  count = 1,
  optional = true
): EffectStep[] => [
  (state, player) =>
...
export const trainerEffects: Record<string, EffectStep[]> = {
  "Ultra Ball": [
  ...
  "Poké Pad": searchDeckToHand(
    "Poké Pad",
    "Search your deck for a Pokémon without a Rule Box",
    (state, c) => isPokemon(state, c) && !hasRuleBox(state, c)
  ),
  ...
  Judge: [
    (state, player) => {
      let next = state;
      for (const p of [player, opponentOf(player)] as Player[]) {
  ...
  "Boss's Orders": [
    ...
    (state, player, picked) => {
      ...
      if (!gust || !ops.active) return state;
  ...
  Crispin: [
    ...
    (state, player, picked) => {
      ...
      const typeOf = (c: CardInstance) => (getCard(state, c.cardId) as any)?.providesType;
  ...
  Dawn: searchDeckToHand(
    "Dawn",
    "Search your deck for a Basic, a Stage 1 and a Stage 2 Pokémon",
    (state, c) => isPokemon(state, c),
    3
  ),
  ...
  "Rare Candy": [
    ...
    (state, player, picked) => {
      ...
      if (!basic || !stage2) return state;
      const ps = state.players[player];
      const evolve = (p: PokemonInPlay) =>
        p.card.instanceId === basic.instanceId
  ...
  "Team Rocket's Petrel": searchDeckToHand(
    "Team Rocket's Petrel",
    "Search your deck for a Trainer card",
    (state, c) => getCard(state, c.cardId)?.type === "trainer"
  ),

  "Team Rocket's Transceiver": searchDeckToHand(
    "Team Rocket's Transceiver",
    "Search your deck for a Team Rocket's Supporter",
    (state, c) => {
      const def = getCard(state, c.cardId);
  ...
  Cyrano: searchDeckToHand(
    "Cyrano",
    "Search your deck for up to 3 Pokémon ex",
    (state, c) => {
      const def = getCard(state, c.cardId) as any;
  ...
  "Pokégear 3.0": lookAtTop(
    "Pokégear 3.0",
    7,
    1,
    "Reveal a Supporter from the top 7 cards of your deck",
    (state, c) => {
      const def = getCard(state, c.cardId);
  ...
  "Bug Catching Set": lookAtTop(
    "Bug Catching Set",
    7,
    2,
    "Reveal up to 2 Grass Pokémon or Basic Grass Energy from the top 7",
    (state, c) => {
      const def = getCard(state, c.cardId) as any;
      if (def?.type === "pokemon") return (def.types ?? []).includes("Grass");
  ...
  "Energy Switch": [
    (state, player) =>
      ask(state, {
        ...
        options: pokemonInPlay(state.players[player]).flatMap((p) =>
          p.attachedEnergy.filter((e) => isBasicEnergy(state, e)).map((e) => e.instanceId)
        ),
        remaining: 1,
        optional: false,
    ...
    (state, player, picked) => {
      ...
      if (!energy) return state;
      const ps = state.players[player];
      const source = pokemonInPlay(ps).find((p) =>
        p.attachedEnergy.some((e) => e.instanceId === energy.instanceId)
      );
      return ask(state, {
        player,
  ...
  "Team Rocket's Ariana": [
    (state, player) => {
      ...
      const allRocket =
        board.length > 0 && board.every((p) => getPokemon(state, p.card.cardId)?.name.startsWith("Team Rocket's"));
      const target = allRocket ? 8 : 5;
      return draw(state, player, Math.max(0, target - ps.hand.length));
    },
  ],

  "Team Rocket's Proton": searchDeckToHand(
    "Team Rocket's Proton",
    "Search your deck for up to 3 Basic Team Rocket's Pokémon",
    (state, c) => {
  ...
  "Rosa's Encouragement": [
    ...
    (state, player, picked) => {
      ...
      if (!target || energies.length === 0) return state;
      const ids = new Set(energies.map((c) => c.instanceId));
      let next = patch(state, player, {
        discard: state.players[player].discard.filter((c) => !ids.has(c.instanceId)),
      });
      for (const energy of energies) next = attachEnergyTo(next, player, energy, target.instanceId);
      return next;
    },
  ],
  ...
  "Jumbo Ice Cream": [
    (state, player) => {
      ...
      if (!active || active.attachedEnergy.length < 3) return state;
  ...
  "Special Red Card": [
    (state, player) => {
      const opp = opponentOf(player);
      const ops = state.players[opp];
      if (ops.hand.length === 0) return state;
...
for (const [name, steps] of Object.entries(trainerEffects)) effectSteps.set(name, steps);

/** Extra play restrictions beyond "it is a Trainer in your hand". */
export const trainerPlayable: Record<string, (state: GameState, player: Player) => boolean> = {
  ...
  "Rosa's Encouragement": (state, player) =>
    state.players[player].prizes.length > state.players[opponentOf(player)].prizes.length,
  "Special Red Card": (state, player) =>
    state.players[player === "p1" ? "p2" : "p1"].prizes.length <= 3,
  ...
  "Energy Switch": (state, player) =>
    [state.players[player].active, ...state.players[player].bench].filter((p) => p).length > 1,
...
export function applyTrainerEffect(state: GameState, trainerName: string, player: Player): GameState {
  const steps = trainerEffects[trainerName];
  if (!steps) return state; // ponytail: unimplemented card is a no-op, tracked by the coverage test
  return steps[0](state, player, []);
}

export const isTrainerImplemented = (name: string) => name in trainerEffects;

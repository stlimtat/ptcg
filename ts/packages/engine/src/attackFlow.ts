import { GameState, PokemonInPlay, PokemonType, Player as Seat } from "./types.js";
...
function log(state: GameState, player: Player, message: string): GameState {
  return { ...state, log: [...state.log, { timestamp: Date.now(), player, message }] };
}

function withPlayer(state: GameState, player: Player, patch: Partial<GameState["players"][Player]>): GameState {
 ...
 */
export function applyKnockout(state: GameState, victim: Player, attacker: Player | null): GameState {
...
export function applyDamage(
  ...
  const damaged: PokemonInPlay = { ...target, damage: target.damage + amount };
...
export function resolveAttack(state: GameState, attacker: Player, attackIndex: number): GameState {
  const attackerPoke = state.players[attacker].active;
  const defender = other(attacker);
  if (!attackerPoke || !state.players[defender].active) return state;

  const attackerDef = getPokemon(state, attackerPoke.card.cardId);
  const attack = attackerDef?.attacks[attackIndex];
  if (!attackerDef || !attack) return state;
...
export function adjustedCost(
  ...
  attack: { cost: PokemonType[] }
): PokemonType[] {
  const adjustment = attackCostAdjustment(state, attacker, source);
  if (adjustment === 0) return attack.cost;
  if (adjustment > 0) return [...attack.cost, ...new Array(adjustment).fill("Colorless" as PokemonType)];
  ...
  for (let i = 0; i < -adjustment; i++) {
    ...
    if (colorless >= 0) cost.splice(colorless, 1);
    else cost.pop();
  }
  return cost;
}

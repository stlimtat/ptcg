...
function encodePokemon(state, poke) {
    if (!poke)
        return new Array(POKEMON_FEATURES).fill(0);
...
function handComposition(state, player) {
    ...
    return [counts.pokemon, counts.basic, counts.energy, counts.item, counts.supporter, counts.tool, counts.stadium].map((n) => n / 10);
}
/** What a single card looks like when it sits in a hand slot or a choice list. */
function encodeCard(state, cardId) {
    if (!cardId)
        return new Array(HAND_CARD_FEATURES).fill(0);
    ...
    return [
        ...
        def.type === "energy" ? 1 : 0,
        def.type === "energy" && def.basic === true ? 1 : 0,
...
function encodeChoiceOption(state, cardId) {
...
function encodeAttacks(state, seat) {
    ...
    for (let i = 0; i < ATTACK_SLOTS; i++) {
        ...
        values.push(1, affordable ? 1 : 0, Math.min(1, damage / 300), them.active.damage + damage >= targetHp ? 1 : 0);
    }
    return values;
}
export function encodeObservation(state, seat) {
    ...
    for (let i = 0; i < BENCH_SLOTS; i++)
        values.push(...encodePokemon(state, me.bench[i] ?? null));
    for (let i = 0; i < BENCH_SLOTS; i++)
        values.push(...encodePokemon(state, them.bench[i] ?? null));
    ...
    for (let i = 0; i < HAND_SLOTS; i++)
        values.push(...encodeCard(state, me.hand[i]?.cardId ?? null));
...
function findCardInstance(state, instanceId) {

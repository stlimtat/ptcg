 ...
 */
export function damageBonus(state, attacker, attack) {
    ...
    if ((m = /Flip a coin until you get tails\. This attack does (\d+) more damage for each heads/.exec(text))) {
        ...
        for (;;) {
            ...
            heads++;
            if (heads > 50)
                break; // guard against a pathological RNG
        }
    ...
    if ((m = /If your opponent's Active Pokémon is a Pokémon ex, this attack does (\d+) more damage/.exec(text))) {
        const def = them.active && getPokemon(state, them.active.card.cardId);
...
export function afterAttack(state, attacker, attack) {
    const text = attack.text ?? "";
    ...
    if (locks.length !== (next.ongoing ?? []).length)
        next = { ...next, ongoing: locks };
...
effectSteps.set("attack:placeCounters", [
    (state, _player, picked, args) => {
        ...
        for (const card of picked) {
            ...
            if (!target)
                continue;
            const bench = next.players[owner].bench.map((p) => p.card.instanceId === card.instanceId ? { ...p, damage: p.damage + 10 } : p);
...
effectSteps.set("attack:snipe", [
    (state, player, picked, args) => {
        ...
        const bench = ps.bench.map((p) => p.card.instanceId === card.instanceId ? { ...p, damage: p.damage + amount } : p);
...
effectSteps.set("attack:ascension", [
    (state, _player, picked, args) => {
        ...
        if (!evolution)
            return state;
        const ps = state.players[owner];
        const evolve = (p) => p.card.instanceId === targetId
...
function knockOutDamagedBench(state, owner, attacker = null) {
    let next = state;
...
export function isAttackTextHandled(text) {
    if (!text.trim())
        return true; // vanilla attack: nothing to do
    return HANDLED_PATTERNS.some((p) => p.test(text));
}
//# sourceMappingURL=attackText.js.map
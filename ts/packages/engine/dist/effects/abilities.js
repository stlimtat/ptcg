...
function takeFromDeck(state, player, picked) {
...
const hasEnergyType = (state, poke, type) => poke.attachedEnergy.some((e) => {
...
export const abilities = {
    ...
    "Run Away Draw": {
        ...
        steps: [
            (state, player, _picked, args) => {
                ...
                if (!source)
                    return next;
                const returned = [source.card, ...source.attachedEnergy, ...source.attachedTools];
        ...
        steps: [
            ...
            (state, player, picked, args) => {
                ...
                if (!energy)
                    return state;
                const ps = state.players[player];
                const attach = (p) => p.card.instanceId === sourceId ? { ...p, attachedEnergy: [...p.attachedEnergy, energy] } : p;
                return patch(state, player, {
            ...
            (state, player, picked, args) => {
                ...
                if (target) {
                    ...
                    if (ops.active?.card.instanceId === target.instanceId) {
                        ...
                        next = patch(next, opp, {
                            bench: ops.bench.map((p) => p.card.instanceId === target.instanceId ? { ...p, damage: p.damage + damage } : p),
                        });
     ...
     */
    "Last-Ditch Catch": {
        usable: (state, player, source) => source.placedOnTurn === state.turn &&
    ...
    "Adrena-Brain": {
        usable: (state, player, source) => hasEnergyType(state, source, "Darkness") &&
        ...
        steps: [
            ...
            (state, player, picked) => {
                ...
                for (const card of picked) {
                    const ps = next.players[player];
                    const shift = (p) => p.card.instanceId === card.instanceId && p.damage >= 10 ? { ...p, damage: p.damage - 10 } : p;
...
for (const [name, impl] of Object.entries(abilities))
    effectSteps.set(name, impl.steps);
export const isAbilityImplemented = (name) => name in abilities;
/** Abilities on a Pokémon that can be activated right now. */
export function usableAbilities(state, player, poke) {
    ...
    return def.abilities
        ...
        if (!impl)
            return false;
        if (used.includes(`${poke.card.instanceId}:${a.name}`))
            return false;
